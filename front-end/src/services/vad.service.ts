...
import { DEFAULT_STT_CONFIG } from "../models/stt.model";

/**
 * Voice Activity Detection processor
 * Powered by Silero VAD using @huggingface/transformers ONNX Runtime
 */
export class VAD {
  ...
  private eventListeners: Partial<
    Record<keyof VADEvents, VADEventCallback<keyof VADEvents>[]>
  > = {};
  public isReady: boolean = false;

  constructor(userConfig: Partial<VADConfig> = {}) {
    // Merge provided user configuration with our default configs from stt.model
    this.config = { ...DEFAULT_STT_CONFIG.vad, ...userConfig };

    // Create buffer based on max duration
    this.buffer = new Float32Array(
      this.config.maxBufferDuration * this.config.sampleRate,
    );
  ...
  public async initialize(): Promise<void> {
    try {
      ...
      this.emit("status", {
        type: "error",
        message: `Failed to load VAD model: ${error?.message || error}`,
      });
      throw error;
    }
  }
   ...
   * Add event listener
   */
  public on<K extends keyof VADEvents>(
    event: K,
    callback: VADEventCallback<K>,
  ): void {
    if (!this.eventListeners[event]) {
      this.eventListeners[event] = [];
    }
    this.eventListeners[event]!.push(
      callback as VADEventCallback<keyof VADEvents>,
    );
  }
   ...
   */
  public off<K extends keyof VADEvents>(
    event: K,
    callback: VADEventCallback<K>,
  ): void {
  ...
  public async processAudio(inputBuffer: Float32Array): Promise<void> {
    ...
    const minSpeechDurationSamples =
      this.config.minSpeechDurationMs * sampleRateMs;
    const maxPrevBuffers = Math.ceil(
      speechPadSamples / this.config.newBufferSize,
    );
    ...
    if (isSpeech) {
      if (!this.isRecording) {
        // Speech just started
        this.emit("speech-start", undefined as any);
  ...
  private async detectSpeech(buffer: Float32Array): Promise<boolean> {
    ...
    const { stateN, output } = await (this.inferenceChain =
      this.inferenceChain.then(() =>
        this.model!({
          input,
  ...
  private processSpeechSegment(overflow?: Float32Array): void {
    ...
    const finalBuffer = new Float32Array(
      prevLength + this.bufferPointer + speechPadSamples,
    );
    ...
    finalBuffer.set(
      this.buffer.slice(0, this.bufferPointer + speechPadSamples),
      offset,
    );

    // Emit the speech segment
    this.emit("speech-end", undefined as any);
